...
void generateRandomTransponentMatrix(int n, int m, int* matrix)
{
	for (int j = 0; j < n; j++)
		for (int i = 0; i < m; i++)
			matrix[n *j + i] = rand() % 100;
}
	...
	while (true)
	{
		...
		if (rank == root)
		{
			...
			if (n < 100)
			{
				...
				cout << "matrix 2:" << endl;
				printTransponentMatrix(n, n, matrix2);
			}
			startTime = MPI_Wtime();
		...
		for (int i = 0; i < rowsCount; i++)
		{
			for (int j = 0; j < n; j++)
			{
				...
				for (int k = 0; k < n; k++)
				{
					tempResult[i * n + j] += matrixPart1[i * n + k] * matrixPart2 [k];
					cout << matrixPart2[k] << endl;
				}
			}
		}
